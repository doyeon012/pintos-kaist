...
/*
구현해야 할 것
1. shutdown_power_off()를 사용하여 pintos 종료
*/
void
halt(void)
{
...
/*
구현해야 할 것
1. 파일 이름과 크기에 해당하는 파일 생성
2. 새 파일을 생성해도 열리지 않습니다.
3. 파일 생성 성공 시 true 반환, 실패 시 false 반환
*/
bool
...
/*
구현해야 할 것
1. 파일 이름에 해당하는 파일을 제거
2. 파일 제거 성공 시 true 반환, 실패 시 false 반환
*/
bool
...
/*
주소유효성 검사 - 유효한 유저 영역인지 확인. 유효하지 않은 경우 프로세스 종료.

구현해야 할 것
1. 주소유효성 검사: 포인터가 가리키는 주소가 사용자 영역(0x8048000~0xc0000000l)인지 확인
2. 잘못된 접근시 프로세스 종료(exit(-1))
*/
void
...
tid_t
exec(const char *cmd_line) // 자식  프로세스 생성 xxx????
{
