...
구현해야 할 것
1. 자식 리스트에 접근하여 프로세스 디스크립터 검색
2. 해당 pid가 존재하면 프로세스 디스크립터 반환
3. 리스트에 존재하지 않으면 NULL 리턴
*/
struct
...
argument_stack(char **parse, int count, void **rsp)
{
	...
	*/
	uintptr_t stack_alignment = (uintptr_t)(*rsp) % 8; 

	if (stack_alignment != 0)
	{
		*rsp -= stack_alignment;		  
		memset(*rsp, 0, stack_alignment); 
	}
...
/*
자식 프로세스가 수행되고 종료될 때까지 부모 프로세스 대기

구현해야 할 것
1. 자식 프로세스의 프로세스 디스크립터 검색
2. 예외 처리 발생시 -1 리턴.
2. 자식 프로세스가 종료되지 않았다면 부모 프로세스 대기
3. 유저 프로세스가 정상적으로 종료시 exit status 반환, 아닐 시 -1 반환
4. 자식 프로세서 디스크립터 삭제
*/
int 
